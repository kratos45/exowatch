...
import { Bot, Terminal, CheckCircle2, ArrowRight, FileText, Download, Sparkles, Send, Layers } from "lucide-react";
...
export default function MultiStepAgentView() {
  ...
  const handleExecuteAgent = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      alert("Erreur lors de l'exécution de l'agent multi-étapes.");
    }
    setIsRunning(false);
  };
      ...
      <div className="lg:col-span-5 flex flex-col gap-4">
        <div className="border-2 border-cyan-200 shadow-[0_0_15px_rgba(0,255,255,0.15)] rounded-xl bg-white p-5 flex flex-col gap-4 h-full">
          <div className="flex items-center justify-between border-b border-cyan-100 pb-3">
            ...
            <span className="text-[10px] text-gray-400 font-bold">EXPORT MARKDOWN / PDF</span>
          </div>

          <div className="space-y-3">
            <div>
              <label className="text-xs font-bold text-gray-700 block mb-1">CIBLE DE MISSION</label>
              <select
                value={reportTarget}
              ...
              >
                ...
                <option value="99942 Apophis (2004 MN4)">99942 Apophis (Menace Critique - 370 m)</option>
                <option value="101955 Bennu (1999 RQ36)">101955 Bennu (Chondrite Carbonée - 490 m)</option>
            ...
            >
              <FileText size={14} /> {isGeneratingReport ? "Génération du Dossier..." : "Générer le Dossier NASA"}
            </button>
          </div>
          ...
          {reportMarkdown ? (
            <div className="flex-1 flex flex-col gap-2 mt-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-green-700">Dossier Technique Prêt</span>
                <button
              ...
              <div className="flex-1 max-h-[380px] overflow-y-auto bg-gray-50 border border-gray-200 p-4 rounded-lg text-[11px] leading-relaxed text-gray-800 whitespace-pre-wrap font-mono">
                {reportMarkdown}
              </div>
            </div>
          ) : (
